...
class Snowman():
    ...
    def __init__(self,color,x,y):
        ...
        self.crit=30
        pygame.draw.circle(self.image, (200,0,0),(50, 100), self.crit) #herz
        self.winkel=0
        self.x=x
        self.y=y
        self.dx=0
...
class Crosshair():
    def __init__(self,color,x,y):
        ...
        pygame.draw.line(self.image,(0,0,255),(10,50),(90,50),1)
        pygame.draw.line(self.image,(0,0,255),(50,10),(50,90),1)
        pygame.draw.circle(self.image,self.color,(50,50),20,1)
        pygame.draw.circle(self.image,self.color,(50,50),30,1)
        self.x=x
        self.y=y
        self.image.set_colorkey((0,0,0))
        self.image.convert_alpha()
...
class Viewer():
    ...
    def __init__(self, maxx=800, maxy=600):
        ...
        self.cross1=Crosshair(color=(200,200,0),x=20,y=20)
        self.cross2=Crosshair(color=(0,0,200), x=-20, y=20)
    def run(self):
        running=True
        while running:
            ...
            for number , j in enumerate (self.joysticks):
                if number== 0:
                    ...
                    x2= j.get_axis(2)
                    y2= j.get_axis(3)
                    self.snowman1.winkel+=x2
                    self.snowman1.winkel=self.snowman1.winkel%360
                    print(self.snowman1.winkel)
                        ...
                        if pushed and b ==7:
                            self.all.append(
                              Ball(color=(255,30,50),boss=self.snowman1,
                                ...
                                dx=10*math.cos(self.snowman1.winkel*math.pi/180) + seconds*self.snowman1.speed*x*3,
                                dy=10*math.sin(self.snowman1.winkel*math.pi/180) + seconds*self.snowman1.speed*y*3,
                                ))
                if number== 1:
                    x= j.get_axis(0)
                    y= j.get_axis(1)
                    x2= j.get_axis(2)
                    y2= j.get_axis(3)
                    self.snowman2.winkel+=x2
                    self.snowman2.winkel=self.snowman2.winkel%360
                    print(self.snowman2.winkel)
                    buttons = j.get_numbuttons()
                    for b in range (buttons):
                        pushed=j.get_button(b)
                        if pushed and b ==7:
                            self.all.append(
                              Ball(color=(255,30,50),boss=self.snowman2,
                                x=self.snowman2.x+40,
                                y=self.snowman2.y+40,
                                dx=10*math.cos(self.snowman2.winkel*math.pi/180) + seconds*self.snowman1.speed*x*3,
                                dy=10*math.sin(self.snowman2.winkel*math.pi/180) + seconds*self.snowman1.speed*y*3,
                                ))
            ...
            pygame.draw.rect(self.screen, (255,0,0), (Viewer.width//2,0,
                                                       balkenweiten[1],10))
            newlist=[]
            #hits
            for ball in self.all:
                ...
                distance=((ball.x-target.x-25)**2+(ball.y-target.y-50)**2)**0.5
            ...
            #fadenkreuz zeichen um schusswinkel anzugeben
            x=self.snowman1.x+100*math.cos(self.snowman1.winkel*math.pi/180)
            y=self.snowman1.y+100*math.sin(self.snowman1.winkel*math.pi/180)
            self.screen.blit(self.cross1.image,(x,y)) 
            x=self.snowman2.x+100*math.cos(self.snowman2.winkel*math.pi/180)
            y=self.snowman2.y+100*math.sin(self.snowman2.winkel*math.pi/180)
            self.screen.blit(self.cross2.image,(x,y)) 
